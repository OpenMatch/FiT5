...
class T5Encoder(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        if config.use_global:
            ...
            self.global_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if use_global and (hidden_state_detach is not None or not grad_detach):

                if i == 9:
                    
                    if grad_detach:
                        hidden_states = torch.cat([hidden_states, hidden_state_detach], dim=0)

                    hidden_states = self.global_layer_norm(hidden_states)
                    ...
                    hidden_states = hidden_states[:doc_size,:,:]
    ...
    def forward(
        ...
        if grad_detach:
            ...
            with torch.no_grad():
                ...
                hidden_states2 = encoder_outputs2[0]
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,
                hidden_state_detach=hidden_states2 if grad_detach else None
            )
        ...
        if grad_detach:
            hidden_states = torch.cat([hidden_states, hidden_states2], dim=0)
