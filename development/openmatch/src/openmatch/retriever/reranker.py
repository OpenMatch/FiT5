...
class RRPredictDataset(IterableDataset):
    ...
    def get_cocondenser_score(self, score):
        score = float(score)
        score_min = 165
        score_max = 190
    ...
    def __iter__(self):
        for qid, did_and_scores in self.run.items():
            for did, scores in did_and_scores.items():
                #print(self.corpus_dataset[did])
                passage_text = self.tokenizer.decode(self.corpus_dataset[did]["input_ids"])
                if "score: Document:" in passage_text:
                    passage_text = passage_text.replace("score: Document:", "score: {} Document:".format(self.get_cocondenser_score(scores)))
                    self.corpus_dataset[did]["input_ids"] = self.tokenizer.encode(passage_text)[:-1]
                yield {
                    "query_id": qid, 
                    "doc_id": did, 
                    **encode_pair(
