...
class SimpleTrainPreProcessor:
    ...
    def get_passage_and_score(self, p, score):
        entry = self.collection[int(p)] if p != "None" else self.collection[0]
        title = entry[self.title_field]
        title = "" if title is None else title
        body = entry[self.text_field]
    ...
    def process_one_have_score(self, train):
        q, pp, pp_score, nn, nn_score = train
        train_example = {
            'query': self.get_query(q),
            'positives': [self.get_passage_and_score(pp[i], pp_score[i]) for i in range(len(pp))],
            'negatives': [self.get_passage_and_score(nn[i], nn_score[i]) for i in range(len(nn))],
        }
