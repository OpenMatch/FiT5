export OUTPUT_DIR=/OpenMatch/experiments # Path to store evaluation results.
export train_job_name=cocodr-base-msmarco # Folder of model files (Placed under OUTPUT_DIR by default).
export model_name=OpenMatch/cocodr-base-msmarco
## *************************************
## GPU setup
TOT_CUDA="0"
CUDAs=(${TOT_CUDA//,/ })
...
#export DATA_DIR=/dataset/beir
# Folders of dataset files (Placed under DATA_DIR by default).
#dataset_name_list=(msmarco-train) #(climate-fever fever bioasq scidocs) #(trec-covid nfcorpus fiqa arguana webis-touche2020 quora scidocs scifact nq hotpotqa signal1m trec-news robust04 dbpedia-entity fever climate-fever bioasq) 

# ## *************************************
# # The CQADupStack dataset contains 12 small datasets.
# ## *************************************
# # Path to store CQADupStack dataset files.
export DATA_DIR=/dataset/beir/cqadupstack
# # Folders of dataset files (Placed under DATA_DIR by default).
...
do
    ...
    elif [ ${dataset_name} == quora ] || [ ${dataset_name} == msmarco ] || [ ${dataset_name} == msmarco-train ]
    then
        export q_max_len=64
        export p_max_len=128
        echo ${infer_job_name}
