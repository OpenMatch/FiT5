...
STEP=10000
CHECKPOINT_DIR="/OpenMatch/checkpoints/msmarco_bm25_4/checkpoint-$STEP"
cp /t5-base-scaled/spiece.model $CHECKPOINT_DIR
task="nfcorpus"
chk="/OpenMatch/checkpoints/msmarco_merge_2/checkpoint-10000/" #$CHECKPOINT_DIR #"/msmarco-10k" #"/pygaggle/pygaggle/checkpoint2/checkpoint-10000/" #"/OpenMatch/checkpoints/msmarco_bm25_4/checkpoint-25000/" #"/OpenMatch/checkpoints/msmarco_bm25/checkpoint-80000/" #"/msmarco-100k"
#dataset_name_list=(trec-covid nfcorpus fiqa arguana webis-touche2020 quora scidocs scifact nq hotpotqa signal1m trec-news robust04 dbpedia-entity fever climate-fever bioasq msmarco) 
...
do
    ...
    then
        ...
        CUDA_VISIBLE_DEVICES=${TOT_CUDA} OMP_NUM_THREADS=1 python -m torch.distributed.launch --nproc_per_node=${CUDA_NUM} --master_port=${PORT} -m openmatch.driver.rerank  \
            ...
            --model_name_or_path $chk\
            --per_device_eval_batch_size 256  \
            --query_path /dataset/beir/cqadupstack/$task/queries.jsonl  \
            --corpus_path /dataset/beir/cqadupstack/$task/corpus.jsonl  \
            --query_template "Query: <text>" \
            --doc_template "Document: <text> Relevant: " \
            --query_column_names id,text  \
            --doc_column_names id,title,text  \
