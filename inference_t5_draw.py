                ...
                with torch.cuda.amp.autocast():
                    if score_token_ids is not None:
                        batch_score, layer_score = model(
                            ...
                            label_for_analyse=rating.to(args.device),
                        )
                    else:
                        batch_score, layer_score = model(
                    ...
                    batch_score = batch_score[:,1176].view(-1).detach().cpu().tolist()
                    #print(batch_loss)
                    #batch_score_softmax = torch.softmax(batch_score[:,1176].view(-1), dim=0).detach().cpu().tolist()
                    layer_score_dict[query_ids[i][0]] = layer_score
                    for (q_id, d_id, b_s) in zip(query_ids[i], doc_ids[i], batch_score):
                        if q_id not in rst_dict:
                            rst_dict[q_id] = {}
                        if d_id not in rst_dict[q_id] or b_s > rst_dict[q_id][d_id][0]:
                            rst_dict[q_id][d_id] = [b_s]
    with open('layer_score_20_no_score.json', 'w') as f:
        json.dump(layer_score_dict, f)
    return rst_dict
...
def main():
    ...
    parser.add_argument('-checkpoint', type=str, default=name + '.bin_step-50000.bin')
    parser.add_argument('-res', type=str, default=name + '_test.trec')
